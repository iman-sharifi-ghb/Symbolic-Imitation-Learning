...
% Safe or Unsafe distances
front_distance_is_safe:-
    front(Car),distanceX(ego,Car,Dx),critical_distance(C),Dx>=C.

front_right_distance_is_safe:-
    front_right(Car),distanceX(ego,Car,Dx),critical_distance(C),Dx>=C.

back_right_distance_is_safe:-
    back_right(Car),distanceX(ego,Car,Dx),critical_distance(C),Dx>=C.

back_distance_is_safe:-
    back(Car),distanceX(ego,Car,Dx),critical_distance(C),Dx>=C.

back_left_distance_is_safe:-
    back_left(Car),distanceX(ego,Car,Dx),critical_distance(C),Dx>=C.

front_left_distance_is_safe:-
    front_left(Car),distanceX(ego,Car,Dx),critical_distance(C),Dx>=C.

front_distance_is_unsafe:- not(front_distance_is_safe).
front_right_distance_is_unsafe:- not(front_right_distance_is_safe).
back_right_distance_is_unsafe:- not(back_right_distance_is_safe).
back_distance_is_unsafe:- not(back_distance_is_safe).
back_left_distance_is_unsafe:- not(back_left_distance_is_safe).
front_left_distance_is_unsafe:- not(front_left_distance_is_safe).

% set the learned heads as dynamic parameters
:-dynamic rlc_is_unsafe/0.
:-dynamic llc_is_unsafe/0.
:-dynamic rlc_is_dangerous/0.
:-dynamic llc_is_dangerous/0.
:-dynamic lk_is_dangerous/0.
:-dynamic rlc_is_better/0.
:-dynamic llc_is_better/0.
:-dynamic reachDesiredSpeed/0.
:-dynamic reachFrontSpeed/0.
:-dynamic brake/0.

% ========================================================================================================
% Load Hypotheses (extracted in the rule induction component)
:- ['hypotheses.pl'].
...
%     front_is_busy2, velocity_x(ego, Vx), Vx<100, llc_is_unsafe, not(rlc_is_unsafe).

% Rule 2: smoothness
critical_number_for_lane_change(5).
lk_is_better:- previous_lane_changes(N), critical_number_for_lane_change(CN), N>CN.

% =============================================================================================
% rule aggregation -> post-processing
take_lk:- not(lk_is_dangerous), lk_is_better.
take_llc:- not(llc_is_unsafe), not(llc_is_dangerous), llc_is_better.
take_rlc:- not(rlc_is_unsafe), not(rlc_is_dangerous), rlc_is_better.

best_action(A):-
    (take_lk->A = lane_keeping;
     take_llc->A = left_lane_change;
     llc_is_better->A = right_lane_change;
                    A = lane_keeping).
...
acceleration_x(Ax):-
    (reachDesiredSpeed;ego_location_is_busy),
